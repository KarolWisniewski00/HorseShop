<x-guest-layout>
    ...
    <section class="pt-20">
        <div class="container mx-auto text-center my-16 relative">
            ...
            <div class="text-5xl relative font-display font-bold bg-clip-text bg-gradient-to-l from-stone-950 to-stone-500  dark:from-stone-50 dark:to-stone-50 text-transparent">SZCZEGÓŁY KOSZYKA</div>
        </div>
        ...
        <div class="container mx-auto px-4 mt-8">
            <div class="mx-auto justify-center md:flex md:space-x-6">
                <div class="rounded-xl md:w-2/3">
                    ...
                    <div class="justify-between mb-6 rounded-xl bg-white p-6 shadow sm:flex sm:justify-start dark:bg-stone-700">
                        <img src="{{asset('asset/photo/'.$item->associatedModel->photo)}}" alt="product-image" class="w-full rounded-xl sm:w-40" />
                        <div class="sm:ml-4 sm:flex sm:w-full sm:justify-between">
                            <div class="mt-5 sm:mt-0">
                                <h2 class="text-lg font-bold text-stone-900 dark:text-stone-50">{{$item->name}}</h2>
                                <p class="mt-1 text-xs text-stone-700 dark:text-stone-400">{{$item->attributes[0]}}</p>
                            </div>
                            <div class="mt-4 flex justify-between sm:space-y-6 sm:mt-0 sm:block sm:space-x-6">
                                <div class="flex items-center border-stone-100">
                                    <form method="POST" action="{{route('product.minus', $item->associatedModel)}}">
                                        ...
                                        <button type="submit" class="cursor-pointer rounded-l bg-stone-100 py-3 px-3.5 duration-100 hover:bg-bone-500 hover:text-bone-50 dark:bg-stone-600 dark:text-stone-50">
                                            -
                                        </button>
                                    </form>
                                    <div class="h-12 w-12 border bg-white text-xs outline-none flex flex-col items-center dark:bg-stone-600 dark:border-stone-500 dark:text-stone-50">
                                        <div class="my-auto w-full text-center">{{$item->quantity}}</div>
                                    </div>
                                    <form method="POST" action="{{route('product.add', $item->associatedModel)}}">
                                        @csrf
                                        <input type="hidden" name="quantity" value="1">
                                        <button type="submit" class="cursor-pointer rounded-r bg-stone-100 py-3 px-3 duration-100 hover:bg-bone-500 hover:text-bone-50 dark:bg-stone-600 dark:text-stone-50">
                                            +
                                        </button>
                                    </form>
                                </div>
                                <div class="flex justify-end space-x-4">
                                    ...
                                    <p class="text-sm dark:text-stone-50">{{$item->associatedModel->price_promo*$item->quantity}} PLN</p>
                                    @php
                                    $count += ($item->associatedModel->price_promo*$item->quantity)
                                    @endphp
                                    @else
                                    <p class="text-sm dark:text-stone-50">{{$item->price*$item->quantity}} PLN</p>
                                    @php
                                    ...
                                    <form method="POST" action="{{route('product.remove', $item->associatedModel)}}">
                                        @csrf
                                        <button type="submit" onclick="return confirm('Czy na pewno chcesz usunąć ten produkt?');"> <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-5 w-5 cursor-pointer duration-150 hover:text-red-500 dark:text-stone-50">
                    ...
                    <img style="max-height:12vh;" class="h-auto max-w-full dark:bg-stone-50 dark:w-fit dark:mx-auto dark:p-4 dark:rounded-xl" alt="" src="{{asset('asset/image/empty-busket.svg')}}">
                        <div class="h4 m-0 p-0 my-3 leading-loose font-semibold dark:text-stone-50">Twój koszyk jest pusty!</div>
                    </div>
                    @endif
                </div>
                <!-- Sub total -->
                <div class="mt-6 h-full rounded-xl border bg-white p-6 shadow md:mt-0 md:w-1/3 dark:bg-stone-700 dark:border-stone-600">
                    <div class="mb-2 flex justify-between">
                        <p class="text-stone-700 dark:text-stone-50">Koszyk</p>
                        <p class="text-stone-700 dark:text-stone-50">{{$count}} PLN</p>
                    </div>
                    ...
                    @if($count == 0)
                    <a class="shadow inline-block py-2 px-6 bg-bone-500 hover:bg-white hover:text-bone-500 w-full text-center my-4 text-white font-bold rounded-xl transition duration-200 dark:bg-bone-600 dark:text-bone-50 dark:hover:bg-stone-800 dark:hover:text-bone-600" href="{{route('shop')}}"><i class="fa-solid fa-shop mr-2"></i>Przejdź do sklepu</a>
                    @else
                    <a class="shadow inline-block py-2 px-6 bg-bone-500 hover:bg-white hover:text-bone-500 w-full text-center my-4 text-white font-bold rounded-xl transition duration-200 dark:bg-bone-600 dark:text-bone-50 dark:hover:bg-stone-800 dark:hover:text-bone-600" href="{{route('order.create')}}"><i class="fa-solid fa-cash-register mr-2"></i>Przejdź do płatności</a>
                    @endif
                </div>
            </div>
        </div>
