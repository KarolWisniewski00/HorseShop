<x-guest-layout>
    ...
    <section class="pt-20">
        <div class="container mx-auto text-center my-16 relative">
            ...
            <div class="text-5xl font-horse relative">Podsumowanie koszyka</div>
        </div>
                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-3">
                    ...
                    <li aria-current="page">
                        <div class="flex items-center">
                            ...
                            <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400">{{ Breadcrumbs::render('busket') }}</span>
                        </div>
                    </li>
                </ol>
            </nav>
        ...
        <div class="container mx-auto px-4 mt-8">
            <div class="mx-auto justify-center md:flex md:space-x-6">
                <div class="rounded-xl md:w-2/3">
                    ...
                    <div class="justify-between mb-6 rounded-xl bg-gray-100 p-6 shadow sm:flex sm:justify-start">
                        <img src="{{asset('asset/image/'.$item->associatedModel->photo)}}" alt="product-image" class="w-full rounded-xl sm:w-40" />
                        <div class="sm:ml-4 sm:flex sm:w-full sm:justify-between">
                            <div class="mt-5 sm:mt-0">
                                <h2 class="text-lg font-bold text-gray-900">{{$item->name}}</h2>
                                <p class="mt-1 text-xs text-gray-700">{{$item->attributes[0]}}</p>
                            </div>
                            <div class="mt-4 flex justify-between sm:space-y-6 sm:mt-0 sm:block sm:space-x-6">
                                <div class="flex items-center border-gray-100">
                                    <form method="POST" action="{{route('product.minus', $item->associatedModel)}}">
                                        ...
                                        <button type="submit" class="cursor-pointer rounded-l bg-gray-100 py-3 px-3.5 duration-100 hover:bg-bone-500 hover:text-bone-50">
                                            -
                                        </button>
                                    </form>
                                    ...
                                    <form method="POST" action="{{route('product.add', $item->associatedModel)}}">
                                        @csrf
                                        <input type="hidden" name="quantity" value="1">
                                        <button type="submit" class="cursor-pointer rounded-r bg-gray-100 py-3 px-3 duration-100 hover:bg-bone-500 hover:text-bone-50">
                                            +
                                        </button>
                                    </form>
                                </div>
                ...
                <!-- Sub total -->
                <div class="mt-6 h-full rounded-xl border bg-gray-100 p-6 shadow md:mt-0 md:w-1/3">
                    <div class="mb-2 flex justify-between">
                        <p class="text-gray-700">Koszyk</p>
                        <p class="text-gray-700">{{$count}} PLN</p>
                    </div>
                    ...
                    @if($count == 0)
                    <a href="{{route('shop')}}" class="block text-center mt-6 w-full rounded-md bg-bone-500 py-1.5 font-medium text-bone-50 hover:bg-bone-600">Przejdź do sklepu</a>
                    @else
                    <a href="{{route('order.create')}}" class="block text-center mt-6 w-full rounded-md bg-bone-500 py-1.5 font-medium text-bone-50 hover:bg-bone-600">Przejdź do płatności</a>
                    @endif
                </div>
            </div>
        </div>
