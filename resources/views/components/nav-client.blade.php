...
<section class="fixed top-0 left-0 right-0 z-10 bg-white shadow">
    <nav class="relative px-4 py-4 flex justify-between items-center bg-transparent container mx-auto">
        ...
        <ul class="hidden absolute top-1/2 left-1/2 transform -translate-y-1/2 -translate-x-1/2 lg:flex lg:mx-auto lg:flex lg:items-center lg:w-auto lg:space-x-6">
            <li><a class="mx-2 text-sm {{ request()->routeIs('index') ? 'text-bone-600 font-bold' : 'text-gray-400 hover:text-gray-500' }}" href="{{route('index')}}">Home</a></li>
            <li><a class="mx-2 text-sm {{ Str::startsWith(request()->path(), 'shop') ? 'text-bone-600 font-bold' : 'text-gray-400 hover:text-gray-500' }}" href="{{route('shop')}}">Sklep</a></li>
            <li><a class="mx-2 text-sm {{ request()->routeIs('about') ? 'text-bone-600 font-bold' : 'text-gray-400 hover:text-gray-500' }}" href="{{route('about')}}">O nas</a></li>
            <li><a class="mx-2 text-sm {{ request()->routeIs('contact') ? 'text-bone-600 font-bold' : 'text-gray-400 hover:text-gray-500' }}" href="{{route('contact')}}">Kontakt</a></li>
        </ul>
        @auth
        <a class="shadow hidden lg:inline-block lg:ml-auto lg:mr-3 py-2 px-6 bg-gray-50 text-bone-500 hover:bg-bone-500 hover:text-white text-sm text-gray-900 font-bold  rounded-xl transition duration-200" href="{{route('profile')}}"><i class="fa-solid fa-user mr-2"></i>Konto</a>
        @else
    ...
    <div class="navbar-menu relative z-50 hidden">
        ...
        <nav class="fixed top-0 left-0 bottom-0 flex flex-col w-5/6 max-w-sm py-6 px-6 bg-white border-r overflow-y-auto">
            ...
            <div>
                <ul>
                    <li class="mb-1">
                        <a class="shadow block p-4 text-sm font-semibold text-gray-400 hover:bg-bone-50 hover:text-bone-600 rounded" href="{{route('index')}}">Home</a>
                    </li>
                    <li class="mb-1">
                        <a class="shadow block p-4 text-sm font-semibold text-gray-400 hover:bg-bone-50 hover:text-bone-600 rounded" href="{{route('shop')}}">Sklep</a>
                    </li>
                    <li class="mb-1">
                        <a class="shadow block p-4 text-sm font-semibold text-gray-400 hover:bg-bone-50 hover:text-bone-600 rounded" href="{{route('about')}}">O nas</a>
                    </li>
                    <li class="mb-1">
                        <a class="shadow block p-4 text-sm font-semibold text-gray-400 hover:bg-bone-50 hover:text-bone-600 rounded" href="{{route('contact')}}">Kontakt</a>
                    </li>
                </ul>
            </div>
            <div class="mt-auto">
