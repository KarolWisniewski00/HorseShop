    <!--footer-->
    <section>
        <footer class="m-4 container mx-auto px-4 my-16">

            <div class="mx-auto  bg-gray-100 rounded-3xl shadow w-full p-4 md:flex md:items-center md:justify-between">
                <span class="text-sm text-gray-500 sm:text-center">© 2023 Karol Wiśniewski. All Rights Reserved.</span>
                <ul class="flex flex-wrap items-center mt-3 text-sm font-medium text-gray-500 sm:mt-0">
                    <li>
                        <a href="{{route('rule')}}" class="mr-4 hover:underline md:mr-4 ">Regulamin</a>
                    </li>
                    <li>
                        <a href="{{route('policy-priv')}}" class="mr-4 hover:underline md:mr-4">Polityka prywatności</a>
                    </li>
                    <li>
                        <a href="{{route('policy-cookie')}}" class="mr-4 hover:underline md:mr-4">Polityka Cookies</a>
                    </li>
                </ul>
            </div>
        </footer>
    </section>