<a href="{{route('product.show',$product)}}" class="w-full product-small h-full flex flex-col justify-between">
    ...
    <div class="overflow-hidden relative bg-gray-100 rounded-xl shadow p-4 mx-16 -mt-12 md:mx-4 relative text-center flex flex-col items-center justify-center">
        ...
        <div class="flex flex-row align-middle items-center justify-center">
            <h5 class="relative m-0 p-0 my-2 mb-4 font-bold text-2xl text-bone-500 line-through">{{$product->price}} zł</h5>
            <h5 class="relative m-0 p-0 my-2 mb-4 mx-2 text-gray-700 product-price">{{$product->price_promo}} zł</h5>
        </div>
        @else
        ...
        <form method="POST" action="{{route('product.add', $product)}}" class="w-full">
            @csrf
            <button type="submit" class="relative duration-200 block shadow w-full text-bone-50 text-xl md:text-xs px-4 py-2 mb-2 leading-loose text-center font-semibold bg-bone-600 hover:bg-gray-50 hover:text-bone-600 rounded-xl"><i class="fa-solid fa-cart-shopping mr-2"></i>Dodaj do koszyka</button>
        </form>
